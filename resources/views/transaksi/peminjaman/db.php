...
function filter_buku(){
  ...
  function filter_siswa(){
    ...
    $.ajax({
          ...
          $.ajax({
             type:'POST',
             url:"{{ route('ajaxRequestTable.filter_table') }}",
             data:{nis:x},
             success:function(data){
               if(data != ""){
                document.getElementById("content-peminjaman").innerHTML = data;
               }else{
                document.getElementById("content-peminjaman").innerHTML = '<tr><td colspan="7" class="text-center text-mute">Tidak Ada Data</td></tr>';
               }
